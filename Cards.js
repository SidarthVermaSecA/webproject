...
const createChatLi = (message, className) => {
  ...
  let chatContent =
    className === "outgoing"
      ? `<p></p>`
      : `<span class="material-symbols-outlined"><i class="fa-solid fa-user"></span><p></p>`;
...
const generateResponse = (chatElement) => {
  const API_URL =
    "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=AIzaSyAQYMNgusd0dulb6qFmn-aSStXq8wGqesc";
...
const handleChat = () => {
  ...
  fetch(url, {
    ...
    .then((result) => {
      ...
      document.getElementById("gemini").textContent = JSON.stringify(
        data_array[0]
      );
    })
    .catch((error) => {
